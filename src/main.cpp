...
int64_t GetBlockValue(int nHeight)
{
	...
	//	nSubsidy = consensus.nMaxMoneyOut - nMoneySupply;
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t nMoneySupply = chainActive.Tip()->nMoneySupply;
    int64_t mNodeCoins = nMasternodeCount * GetMNCollateral() * COIN;
...
{
    util::ThreadRename("schillingcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Track zSCH money supply in the block index
    if (!UpdateZSCHSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSCH supply for block=%s height=%d", __func__,
    ...
    if (isPoS) {
        ...
        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if (!stakeIn.IsZerocoinSpend()) {
                schInputs.push_back(stakeIn);
            }
        }
        const bool hasSCHInputs = !schInputs.empty();
...
std::string GetWarnings(std::string strFor)
{
    ...
    if (fLargeWorkForkFound) {
        ...
        strStatusBar = strRPC = _("Warning: Your blockchain doesn't match the required checkpoints! You have forked, please resync Schilling Coin.");
    }
