...
bool RecalculateSCHSupply(int nHeightStart, bool fSkipZsch)
{
    ...
    uiInterface.ShowProgress(_("Recalculating SCH supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating SCH supply..."), percent);
        }
        ...
        // Rewrite zSCH supply too
        if (!fSkipZsch && pindex->nHeight >= consensus.height_start_ZC) {
            UpdateZSCHSupply(block, pindex);
        }
...
bool UpdateZSCHSupply(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Remove spends from zSCH supply
