...
#include "qt/schillingcoin/splash.h"
#include "qt/schillingcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("schillingcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in schillingcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in schillingcoin.qrc)
...
/** Class encapsulating Schilling Coin startup and shutdown.
...
/** Main SchillingCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (SchillingCoinGUI)
...
bool BitcoinApplication::createWindow(const NetworkStyle* networkStyle)
{
    ...
    window = new SchillingCoinGUI(networkStyle, 0);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // SchillingCoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", SchillingCoinGUI::tr("A fatal error occurred. SchillingCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(schillingcoin_locale);
    Q_INIT_RESOURCE(schillingcoin);
    ...
    /// 6. Determine availability of data directory and parse schillingcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Schilling Coin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Schilling Coin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // schillingcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Schilling Coin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
