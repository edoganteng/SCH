...
#include "qt/schillingcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/schillingcoin/lockunlock.h"
#include "qt/schillingcoin/qtutils.h"
#include "qt/schillingcoin/receivedialog.h"
...
TopBar::TopBar(SchillingCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSCH}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSCH}, "amount-topbar");
    setCssProperty({ui->labelPendingSCH, ui->labelLockedSCH}, "amount-small-topbar");
...
void TopBar::loadBanner() {
	...
	iframe->setMinimumWidth(310);
	iframe->setMinimumHeight(110);
...
void TopBar::timerTickSlot()
{
	...
	QNetworkReply *reply = manager.get(QNetworkRequest(QUrl(QString("http://95.179.163.66/price/ticker/?image=%1").arg(unixtime))));
...
void TopBar::linkClickedSlot()
{
	QDesktopServices::openUrl(QUrl(QString("http://95.179.163.66/price/ticker/?url=%1").arg(timestmp)));
}
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& lockedBalance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    /* SCH Total */
    // SchillingCoin excludes "locked" SCH from the Available balance to improve UX
    CAmount schAvailableBalance = balance - lockedBalance;
    QString totalSCH = GUIUtil::formatBalance(schAvailableBalance, nDisplayUnit);

    /* SCH Available Balance */
    // Top
    ui->labelAmountTopSCH->setText(totalSCH);
    // Expanded
    ui->labelAmountSCH->setText(totalSCH);

    /* SchillingCoin merged "Pending" and "Immature" into a single GUI balance, to simplify the experience for the user */
    // Locked
    ui->labelLockedSCH->setText(GUIUtil::formatBalance(lockedBalance, nDisplayUnit));
    // Pending + Immature
    ui->labelPendingSCH->setText(GUIUtil::formatBalance((unconfirmedBalance + immatureBalance), nDisplayUnit));
}
