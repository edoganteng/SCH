...
#include "qt/schillingcoin/forms/ui_addresseswidget.h"
#include "qt/schillingcoin/addresslabelrow.h"
#include "qt/schillingcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(SchillingCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter SchillingCoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress schAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(schAdd.Get(), label.toUtf8().constData(),
                schAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
