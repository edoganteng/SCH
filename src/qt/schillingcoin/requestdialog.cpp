...
#include "qt/schillingcoin/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a SchillingCoin address, you can create a payment request, bundling up more information."));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 SCH");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " SCH";
        } else {
