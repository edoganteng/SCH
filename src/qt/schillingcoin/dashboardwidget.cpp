...
#include "qt/schillingcoin/dashboardwidget.h"
#include "qt/schillingcoin/forms/ui_dashboardwidget.h"
#include "qt/schillingcoin/sendconfirmdialog.h"
#include "qt/schillingcoin/txrow.h"
...
DashboardWidget::DashboardWidget(SchillingCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    ui->labelMessage->setText(tr("Amount of SCH earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareSCH, "square-chart-sch");
    setCssProperty(ui->labelSCH, "text-chart-sch");
    ...
    ui->labelAmountSCH->setText("0 SCH");
    setCssProperty(ui->labelAmountSCH, "text-stake-sch-disable");
    ...
    ui->btnHowTo->setText(tr("How to get SCH"));
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair SCH and MN rewards
...
void DashboardWidget::onChartRefreshed() {
    ...
    set0->setColor(QColor(32, 146, 61));
    ...
    if (chartData->totalStakes > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountSCH, "text-stake-sch");
    } else {
        setCssProperty(ui->labelAmountSCH, "text-stake-sch-disable");
    }
    forceUpdateStyle({ui->labelAmountSCH});
    ui->labelAmountSCH->setText(GUIUtil::formatBalance(chartData->totalStakes, nDisplayUnit) + " (Stakes)");
