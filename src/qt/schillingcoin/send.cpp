...
#include "qt/schillingcoin/send.h"
#include "qt/schillingcoin/forms/ui_send.h"
#include "qt/schillingcoin/addnewcontactdialog.h"
#include "qt/schillingcoin/qtutils.h"
#include "qt/schillingcoin/sendchangeaddressdialog.h"
#include "qt/schillingcoin/optionbutton.h"
#include "qt/schillingcoin/sendconfirmdialog.h"
#include "qt/schillingcoin/myaddressrow.h"
...
SendWidget::SendWidget(SchillingCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    ui->labelAmountSend->setText("0.00 SCH");
    ...
    setCssProperty(coinIcon, "coin-icon-sch");
...
void SendWidget::onSCHSelected(bool _isSCH){
    isSCH = _isSCH;
    setCssProperty(coinIcon, "coin-icon-sch");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress schAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(schAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(schAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
