...
void closeDialog(QDialog *widget, SchillingCoinGUI *gui){
    widget->setWindowFlags(Qt::CustomizeWindowHint);
    widget->setAttribute(Qt::WA_TranslucentBackground, true);
    QPropertyAnimation* animation = new QPropertyAnimation(widget, "pos");
    animation->setDuration(300);
...
bool openDialogWithOpaqueBackgroundY(QDialog *widget, SchillingCoinGUI *gui, double posX, int posY){
    widget->setWindowFlags(Qt::CustomizeWindowHint);
    widget->setAttribute(Qt::WA_TranslucentBackground, true);
    QPropertyAnimation* animation = new QPropertyAnimation(widget, "pos");
    animation->setDuration(300);
...
bool openDialogWithOpaqueBackground(QDialog *widget, SchillingCoinGUI *gui, double posX){
    return openDialogWithOpaqueBackgroundY(widget, gui, posX, 5);
}

bool openDialogWithOpaqueBackgroundFullScreen(QDialog *widget, SchillingCoinGUI *gui){
    widget->setWindowFlags(Qt::CustomizeWindowHint);
    widget->setAttribute(Qt::WA_TranslucentBackground, true);
...
QColor getRowColor(bool isLightTheme, bool isHovered, bool isSelected){
    if (isLightTheme) {
        if (isSelected) {
            return QColor("#2589FEAF");
        } else if (isHovered) {
            return QColor("#25bababa");
        } else {
            return QColor("#f1f2f2");
        }
    } else {
        if (isSelected) {
            return QColor("#2589FEAF");
