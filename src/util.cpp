...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "schillingcoin" is a composite category enabling all SchillingCoin-related debug output
            if (ptrCategory->count(std::string("schillingcoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SchillingCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SchillingCoin
// Mac: ~/Library/Application Support/SchillingCoin
// Unix: ~/.schillingcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SchillingCoin";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty schillingcoin.conf if it does not exist
