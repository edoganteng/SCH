...
class ZerocoinSpendTest(SchillingcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zsch_bal, sch_bal):
            zsch_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zsch_bal)
            sch_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], sch_bal)
            return zsch_bal, sch_bal
        ...
        zsch_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zsch_balance, 6666)
        ...
        zsch_balance, balance = check_balances(denom_2, zsch_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zsch_balance, balance = check_balances(denom_3, zsch_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zsch",
        ...
        _, _ = check_balances(denom_1, zsch_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
