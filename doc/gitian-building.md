...
*Setup instructions for a gitian build of Schilling Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SchillingCoin
...
to the SchillingCoin GitHub release page.
...
- [Building Schilling Coin](#building-schillingcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/schillingcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/schillingcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/schillingcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/schillingcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/schillingcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/schillingcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/schillingcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@schillingcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/schillingcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/schillingcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/schillingcoin-osx-*-build.assert
```
