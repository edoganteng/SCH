UNIX BUILD NOTES
====================
Some notes on how to build Schilling Coin in Unix.

Note
---------------------
Always use absolute paths to configure and compile Schilling Coin and the dependencies,
...
memory available when compiling Schilling Coin. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~schillingcoin/+archive/schillingcoin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:schillingcoin/schillingcoin
    sudo apt-get update
...
To build Schilling Coin without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build schillingcoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a schillingcoin-qt executable will be
built by default.
...
Security
--------
To help make your Schilling Coin installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Schilling Coin should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, Schilling Coin may be compiled in
