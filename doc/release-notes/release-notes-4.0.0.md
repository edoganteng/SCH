Schilling Coin version *v4.0.0* is now available from:  <https://github.com/schillingcoin-project/schillingcoin/releases>
...
Schilling Coin v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
You can read more details about this extensive work in ([PR #954](https://github.com/SchillingCoin-Project/SchillingCoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/SchillingCoin-Project/SchillingCoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [SchillingCoin Wiki](https://github.com/SchillingCoin-Project/SchillingCoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/SchillingCoin-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/SchillingCoin-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/SchillingCoin-Project/SchillingCoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 SCH, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zSCH Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zSCH have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zSCH with public spends version 4 ([PR #936](https://github.com/SchillingCoin-Project/SchillingCoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zSCH), version 4 spends will also be public. A full technical writeup is available on the [SchillingCoin Wiki](https://github.com/SchillingCoin-Project/SchillingCoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/SchillingCoin-Project/SchillingCoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/SchillingCoin-Project/SchillingCoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/SchillingCoin-Project/SchillingCoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/SchillingCoin-Project/SchillingCoin/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-zsch-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - SCH balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - SCH balance held in cold staking addresses.
...
Arguments:
1. "stakingaddress"      (string, required) The schillingcoin staking address to delegate.
2. "amount"              (numeric, required) The amount in SCH to delegate for staking. eg 100
3. "owneraddress"        (string, optional) The schillingcoin address corresponding to the key that will be able to spend the stake.
  ...
  "vout" : [              (array of json objects)
     {
       ...
       "scriptPubKey" : {          (json object)
         ...
         "addresses" : [           (json array of string)
           "schillingcoinaddress"        (string) schillingcoin address
           ,...
         ]
       }
     }
...
Result:
amount              (numeric) The total amount in SCH received for this account in P2CS contracts.
```
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~schillingcoin) repository, we are now offering a [Snap package](https://snapcraft.io/schillingcoin-core) as quick way to install and update a SchillingCoin wallet.
...
- #936 `12a6b704b6` [zSCH] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for SchillingCoin v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for SchillingCoin v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open schillingcoin.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong SchillingCoin website (NoobieDev12)
